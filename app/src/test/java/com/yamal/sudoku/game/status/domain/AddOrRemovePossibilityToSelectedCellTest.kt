...
class AddOrRemovePossibilityToSelectedCellTest {

    private val currentGame: CurrentGame = mock()
    private val gameStatusRepository: GameStatusRepository = mock()
    private val addOrRemovePossibilityToSelectedCell =
        AddOrRemovePossibilityToSelectedCell(
            currentGame,
            gameStatusRepository,
        )

    @Test
    fun `should add possibility and save new updated board when game has not finished`() {
        givenGameHasNotFinished()
        givenAnyCurrentBoard()

        addOrRemovePossibilityToSelectedCell(ANY_VALUE)

        verify(currentGame).onPossibilityAddedOrRemoved(ANY_VALUE)
        verify(gameStatusRepository).saveBoard(ANY_BOARD)
    }

    @Test
    fun `should not add possibility and save new updated board when game has finished`() {
        givenGameHasFinished()

        addOrRemovePossibilityToSelectedCell(ANY_VALUE)

        verify(currentGame, never()).onPossibilityAddedOrRemoved(any())
    ...
    private companion object {
        val ANY_VALUE = SudokuCellValue.FIVE
